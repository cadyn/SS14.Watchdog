...
RUN dotnet publish -c release -r linux-musl-x64 -o /app --no-self-contained --no-restore
...
WORKDIR /app
COPY --from=build /app ./SS14.Watchdog/bin/Release/net6.0/linux-musl-x64/publish
RUN mv SS14.Watchdog/bin/Release/net6.0/linux-musl-x64/publish ./ \
  && rm -rf SS14.Watchdog \
  && mv publish/* ./ \
  && rm -rf publish \
